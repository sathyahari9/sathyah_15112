...
####################################
def gameBlocks(img,contents,coordinates):
    colors = {
        (0,100):(0,0,255),
        (75,125):(0,255,0),
        (100,200):(255,0,0),
    ...
    color = colors[key]
    cv2.rectangle(img,(listPlay[-1][0],350),(listPlay[-1][1],400),color)
    if coordinates[-1] == listPlay[-1]:
        listPlay.pop()
...
def playGame():
    ...
    while True:
        ...
        keyID = 0
        gameBlocks(img,contents,coordinates)
        for i in range(0,len(conts)):
            ...
            if 0 <= cx1 <= 75 and 400 <= cy1:
                keyID = 1
                _thread.start_new_thread(play_audio,(keyID,keys,))
...
if __name__ == "__main__":
    ...
    canvas.pack()
    button1 = Button(root,text="Play Game",command=video)
